# Copyright 2016 Akretion (https://www.akretion.com).
...
class SaleOrder(models.Model):
    ...
    def _prepare_invoice(self, order, lines):
        ...
        pos_anonym_journal = self.env.context.get("pos_anonym_journal", False)
...
class PosOrder(models.Model):
    ...
    def create_from_ui(self, orders):
        ...
        for tmp_order in orders_to_save:
            ...
            if session.sequence_number <= ui_order["sequence_number"]:
                session.write({"sequence_number": ui_order["sequence_number"] + 1})
                session.refresh()
            ...
            if payments and not float_is_zero(ui_order["amount_return"], prec_acc):
                ...
                if not cash_journal:
                    ...
                    cash_journals = self.env["account.journal"].search(
                        [("type", "=", "cash"), ("id", "in", list(journal_ids))],
                        limit=1,
                    )
    ...
    def _prepare_payment_vals(self, order_id, data):
        ...
        args = {
            ...
            "partner_id": order.partner_id
            and (
                self.env["res.partner"]._find_accounting_partner(order.partner_id).id
                or False
            ),
        }
        ...
        args["account_id"] = (
            (
                order.partner_id
                and order.partner_id.property_account_receivable
        ...
        if not args["account_id"]:
            if not args["partner_id"]:
                ...
                msg = _(
                    "There is no receivable account defined "
        ...
        if not (journal_id or statement_id):
            raise UserError(_("No statement_id or journal_id passed to the method!"))
...
class PosSession(models.Model):
    ...
    @api.multi
    def _get_so_domains(self, vals, anonym_partner_id, anonym_order=True):
        self.ensure_one()
    ...
    def _get_generated_invoice(
        self, partner_id=False, grouped=False, anonym_order=False, anonym_journal=False
    ):
        domains = {}
    ...
    def _generate_invoice(
        self, partner_id=False, grouped=False, anonym_order=True, anonym_journal=True
    ):
        sale_obj = self.env["sale.order"]
        domains = {}
        domains = self._get_so_domains(domains, partner_id, anonym_order=anonym_order)
    ...
    def _reconcile_invoice_with_pos_payment(self, invoices):
        if not invoices:
            return False
